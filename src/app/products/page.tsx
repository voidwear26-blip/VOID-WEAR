    ...
    <div className="pt-48 pb-32 bg-transparent min-h-screen">
      <div className="container mx-auto px-6 md:px-10">
        <div className="space-y-6 mb-24 max-w-4xl">
          ...
          <p className="text-white/70 tracking-[0.4em] text-xs uppercase leading-relaxed font-light max-w-2xl">
        ...
        <div className="flex flex-col md:flex-row gap-8 mb-16 items-start md:items-center justify-between border-y border-white/5 py-10">
          <div className="relative w-full md:w-[400px] group">
            <Search className="absolute left-6 top-1/2 -translate-y-1/2 w-4 h-4 text-white/40 group-focus-within:text-white transition-colors" />
            <Input 
              placeholder="SEARCH THE ASSEMBLAGE..." 
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              className="bg-white/5 border-white/10 h-14 pl-14 rounded-none text-[10px] tracking-[0.4em] focus-visible:ring-0 focus-visible:border-white/40 font-black text-white uppercase placeholder:text-white/20 transition-all"
            />
          </div>

          <div className="flex items-center gap-6 w-full md:w-auto">
            <div className="hidden sm:flex items-center gap-2 text-[9px] tracking-[0.4em] text-white/60 uppercase font-black mr-2">
              <SlidersHorizontal className="w-3 h-3" />
              SORT:
            </div>
            <Select value={sortBy} onValueChange={(val) => setSortBy(val as SortOption)}>
              <SelectTrigger className="w-full md:w-64 bg-white/5 border-white/10 rounded-none h-14 text-[9px] tracking-[0.4em] uppercase focus:ring-0 text-white font-black transition-all hover:bg-white/10">
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-64 text-center space-y-12 opacity-80 border border-dashed border-white/10">
            <Package className="w-20 h-20 stroke-[0.5px] text-white/20" />
            <div className="space-y-4">
              <p className="text-[12px] tracking-[1.5em] uppercase font-black text-white">NO MODULES LOGGED</p>
              <p className="text-[9px] tracking-[0.5em] uppercase max-w-md mx-auto text-white/40 font-bold">
